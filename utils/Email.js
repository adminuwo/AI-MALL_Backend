...
export const sendFeedbackEmail = async (feedback) => {
  try {
    ...
    const response = await resend.emails.send({
      from: `AIVA Feedback <${process.env.EMAIL}>`,
      to: ['admin@uwo24.com'],
      ...
      html: `
        <div style="font-family: sans-serif; padding: 20px; border: 1px solid #eee; border-radius: 10px;">
          ...
          <hr />
          <p style="font-size: 12px; color: #666;">Generated by AIVA Feedback System</p>
        </div>
      `
    });
...
export const sendContactAdminEmail = async (data) => {
  try {
    const response = await resend.emails.send({
      from: `AIVA Contact <${process.env.EMAIL}>`,
